...
char *structs_int_ascify(const struct structs_type *type, const void *data)
{
...
int structs_int_binify(const struct structs_type *type,
		       ...
		       char *ebuf, size_t emax)
{
	...
	if (eptr == ascii) {
		strncpy(ebuf, "invalid integer value", emax);
		errno = EINVAL;
		return (-1);
	}
	while (isspace(*eptr))
	...
	if ((errno == ERANGE && (value == LLONG_MIN || value == LLONG_MAX))
		...
		strncpy(ebuf, "integer value is out of range", emax);
		errno = EINVAL;
		return (-1);
	}
