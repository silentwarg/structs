...
int structs_float_equal(const struct structs_type *type,
...
char *structs_float_ascify(const struct structs_type *type, const void *data)
{
	const int ftype = type->args[0].i;
	char buf[32];
...
int structs_float_binify(const struct structs_type *type,
			 ...
			 char *ebuf, size_t emax)
{
	...
	while (isspace(*eptr))
		eptr++;
	if (*eptr != '\0') {
	invalid:	strncpy(ebuf, "invalid floating point value", emax);
		errno = EINVAL;
		return (-1);
	}
	...
	if (errno == ERANGE) {
		if (value == 0.0) {
		underflow:		strncpy(ebuf, "floating point value underflow",
						emax);
			errno = EINVAL;
			return (-1);
		}
		if (value == HUGE_VAL || value == -HUGE_VAL) {
		overflow:		strncpy(ebuf, "floating point value overflow",
						emax);
			errno = EINVAL;
			return (-1);
		}
