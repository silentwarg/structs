...
/*******************************************************************************
    ...
    struct fubar {
	struct structs_arraylist;// array of strings
    };
...
typedef int structs_binify_t(const struct structs_type *type,
			     const char *ascii, void *data, char *ebuf,
			     size_t emax);
 ...
 * stored in a dynamically allocated buffer allocated and "code"
 * must be filled in with the buffer's pointer and length.
 *
...
/*
 * Callback function type used to log errors
 */
typedef void structs_logger_t(int sev, const char *fmt, ...);

/* Special pre-defined loggers */
#define STRUCTS_LOGGER_NONE  ((structs_logger_t *)0)	/* discard output */
#define STRUCTS_LOGGER_STDERR ((structs_logger_t *)1)	/* log to stderr */
#define STRUCTS_LOGGER_TRACE ((structs_logger_t *)2)	/* log using trace */

structs_logger_t structs_null_logger;
structs_logger_t structs_stderr_logger;
structs_logger_t structs_trace_logger;
...
extern int structs_equal(const struct structs_type *type,
			 const char *name, const void *data1,
			 const void *data2);
 ...
 */
extern const struct structs_type *structs_find(const struct structs_type
...
/*
 * Get the ASCII form of an item, in a string allocated.
 *
 ...
 * allocated and described by "code".
 *
...
extern int structs_set_binary(const struct structs_type *type,
			      const char *name,
			      const struct structs_data *code, void *data,
			      char *ebuf, size_t emax);
 ...
 * and then the array itself; all memory is allocated.
 *
