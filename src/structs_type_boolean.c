...
static char *structs_boolean_ascify(const struct structs_type *type,
				    const void *data)
{
	int truth;
	...
	truth =
		type->args[0].
		i ? *((unsigned int *)data) : *((unsigned char *)data);
	return (strdup(boolean_strings[type->args[1].i][!!truth]));
}

static int structs_boolean_binify(const struct structs_type *type,
				  ...
				  char *ebuf, size_t emax)
{
	...
	if (boolean_strings[i][0] == NULL) {
		strncpy(ebuf, "invalid Boolean value", emax);
		errno = EINVAL;
		return (-1);
	}
	if (type->args[0].i)
		*((unsigned int *)data) = truth;
	else
		*((unsigned char *)data) = truth;
	return (0);
}
