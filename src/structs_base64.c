 ...
 */
struct filter *b64_encoder_create(const char *cmap)
{
	...
	/* Create object */
	if ((enc = calloc(1, sizeof(*enc))) == NULL)
		return (NULL);
	memset(enc, 0, sizeof(*enc));

	/* Copy character map */
	if ((enc->cmap = strdup(cmap)) == NULL) {
		free(enc);
		return (NULL);
	}
 ...
 */
void b64_encoder_destroy(struct filter **filterp)
{
 ...
 */
int b64_encoder_write(struct filter *filter, const void *data, int len)
{
 ...
 */
int b64_encoder_read(struct filter *filter, void *data, int len)
{
	struct b64_encoder *const enc = (struct b64_encoder *)filter;
	int r;
 ...
 */
int b64_encoder_end(struct filter *filter)
{
	struct b64_encoder *const enc = (struct b64_encoder *)filter;
	char buf[4];
	int r;
 ...
 */
static int b64_encoder_convert(struct filter *filter, int num, int forward)
{
	if (forward)
		return (((num * 4) + 2) / 3);
	else
...
static void b64_encoder_encode(struct b64_encoder *enc, char *buf)
{
	...
	buf[0] = enc->cmap[(unsigned char)buf[0]];
	buf[1] = ((enc->dbuf[0] << 4) & 0x30) | ((enc->dbuf[1] >> 4) & 0x0f);
	buf[1] = enc->cmap[(unsigned char)buf[1]];
	buf[2] = ((enc->dbuf[1] << 2) & 0x3c) | ((enc->dbuf[2] >> 6) & 0x03);
	buf[2] = enc->cmap[(unsigned char)buf[2]];
	buf[3] = enc->dbuf[2] & 0x3f;
	buf[3] = enc->cmap[(unsigned char)buf[3]];
}
...
static int b64_encoder_output(struct b64_encoder *enc, const char *buf)
{
	if (enc->olen + 4 > enc->osize) {
		...
		if ((new_obuf = realloc(enc->obuf, new_osize)) == NULL)
			return (-1);
		enc->obuf = new_obuf;
		enc->osize = new_osize;
	}
...
static void b64_decoder_decode(struct b64_decoder *dec, unsigned char *buf);
static int b64_decoder_output(struct b64_decoder *dec, unsigned char *buf,
			      int len);

/*
 * Create a new base-64 decoder with optional custom character set.
 */
struct filter *b64_decoder_create(const char *cmap, int strict)
{
 ...
 */
void b64_decoder_destroy(struct filter **filterp)
{
 ...
 */
int b64_decoder_write(struct filter *filter, const void *data, int len)
{
	...
	while (total < len) {
		...
		for (; dec->coff < 4 && total < len; total++) {
			const unsigned char val =
			    dec->cmap[((unsigned char *)data)[total]];
 ...
 */
int b64_decoder_read(struct filter *filter, void *buf, int len)
{
	struct b64_decoder *const dec = (struct b64_decoder *)filter;
	int r;
 ...
 * Mark end of written data.
 */
int b64_decoder_end(struct filter *filter)
{
	struct b64_decoder *const dec = (struct b64_decoder *)filter;
	unsigned char buf[3];
	int len;
 ...
 * Convert byte count to read before and after filter.
 */
static int b64_decoder_convert(struct filter *filter, int num, int forward)
{
	if (forward)
		return (((num * 3) + 3) / 4);
	else
			      ...
			      int len)
{
	if (dec->olen + len > dec->osize) {
		...
		unsigned char *new_obuf;

		if ((new_obuf = realloc(dec->obuf, new_osize)) == NULL)
			return (-1);
		dec->obuf = new_obuf;
		dec->osize = new_osize;
	}
