...
struct xmlinput_stackframe {
	...
	unsigned char combined;	/* was a combined tag */
};
 ...
 */
int structs_xml_input(const struct structs_type *type,
		      const char *elem_tag, char **attrp,
		      FILE * fp, void *data, int flags,
		      structs_logger_t * logger)
{
	...
	if (ctx->attrp != NULL && (*ctx->attrp = strdup("")) == NULL)
		goto done;
	...
	if ((ctx->info = calloc(1, sizeof(*ctx->info))) == NULL) {
		esave = errno;
				    ...
				    const XML_Char ** attrs)
{
	...
	if (attrs[0] != NULL) {
		sev =
			(info->flags & STRUCTS_XML_LOOSE) ==
			0 ? LOG_ERR : LOG_WARNING;
		(*info->logger) (sev,
				 "line %d:%d: element \"%s\""
				   ...
				   void **datap)
{
	...
	case STRUCTS_TYPE_UNION:
	{
		...
		if ((type =
		     structs_find(type, name, (const void **)&data,
				  1)) == NULL) {
			if (errno == ENOENT) {
				sev = (info->flags & STRUCTS_XML_LOOSE)
					== 0 ? LOG_ERR : LOG_WARNING;
				(*info->logger) (sev,
	...
	case STRUCTS_TYPE_ARRAY:
	{
		...
		if (strcmp(name, elem_name) != 0) {
			(*info->logger) (LOG_ERR,
					 "line %d:%d: expected element"
					 " \"%s\" instead of \"%s\"",
					 XML_GetCurrentLineNumber
					 (info->p),
					 XML_GetCurrentColumnNumber
					 (info->p), elem_name, name);
			info->error = EINVAL;
			return;
		}
				       ...
				       const XML_Char * s, int len)
{
	...
	if ((mem = realloc(frame->s, frame->s_len + len + 1)) == NULL) {
		info->error = errno;
...
static int structs_xml_output_sub(const struct structs_type *type,
				  ...
				  const char *attrs, FILE * fp,
				  const char **elems, const char *posn,
				  int flags, int depth);
static void structs_xml_output_openelem(FILE * fp, int depth, const char *tag,
					const char *attrs);
 ...
 * Note: it is safe for the calling thread to be canceled.
 */
int structs_xml_output(const struct structs_type *type, const char *elem_tag,
		       const char *attrs, const void *data, FILE * fp,
		       const char **elems, int flags)
{
	...
	return (structs_xml_output_sub(type, data, elem_tag, attrs, fp,
				       elems, "", flags | STRUCTS_XML_SHOWONE,
				       0));
}
				  ...
				  int flags, int depth)
{
	...
	if (depth > 0
		...
		if ((init_value = calloc(1, type->size)) == NULL)
			return (-1);
	...
	case STRUCTS_TYPE_UNION:
	{
		...
		asprintf(&sposn, "%s%s%s", posn,
			 *posn != '\0' ? separator_string : "",
			 field->name);
		if (sposn == NULL)
			return (-1);
		...
		r = structs_xml_output_sub(field->type, un->un,
					   field->name, NULL, fp,
					   elems, sposn, flags,
					   depth + 1);
	...
	case STRUCTS_TYPE_STRUCTURE:
	{
		...
		for (field = type->args[0].v; field->name != NULL;
			...
			pthread_cleanup_push
				(structs_xml_output_cleanup, sposn);
	...
	case STRUCTS_TYPE_ARRAY:
	{
		...
		for (i = 0; i < ary->length; i++) {
			...
			r = structs_xml_output_sub(etype,
						   (char *)ary->elems +
						   (i * etype->size),
						   elem_name, NULL, fp,
						   elems, sposn, flags,
 ...
 */
static void structs_xml_output_openelem(FILE * fp, int depth,
